...
from game.game.manager import GameManager, SCORE_MAX, SCORE_MIN
...
_FAKE_PLAYLIST = {"name": "Test Playlist", "image_url": "https://img.example.com/pl.jpg"}


@pytest.fixture(autouse=True)
def mock_playlist_details():
    """Mock Spotify playlist lookup and remove inter-round delays for speed."""
    with patch(
        "game.consumers.get_playlist_details",
        new=AsyncMock(return_value=_FAKE_PLAYLIST),
    ):
        with patch("game.game.manager.ROUND_BREAK", 0):
            yield
    ...
    async def test_single_player_completes_one_round(self):
        ...
        with patch(
            ...
            with patch.object(GameManager, "_stream_audio", new=AsyncMock(return_value=None)):
                with patch.object(GameManager, "_prefetch_url", new=AsyncMock(return_value=None)):
                    await comm.send_json_to({"type": "room.start"})
                    ...
                    # 6. Receive song.correct with SCORE_MAX points (first and only guesser)
                    r = await recv_until(comm, "song.correct")
                    assert r["data"]["points"] == SCORE_MAX
        ...
        with patch(
            ...
            with patch.object(GameManager, "_stream_audio", new=AsyncMock(return_value=None)):
                with patch.object(GameManager, "_prefetch_url", new=AsyncMock(return_value=None)):
                    ...
                    assert r_a["data"]["points"] >= SCORE_MIN
                    ...
                    assert r_b["data"]["points"] >= SCORE_MIN
                    ...
                    # Final scoreboard has scores for both players
                    final_scores = r_end_a["data"]["state"]["scoreboard"]
                    assert final_scores.get("Alice", 0) >= SCORE_MIN
                    assert final_scores.get("Bob", 0) >= SCORE_MIN
